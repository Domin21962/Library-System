...
function ensureAdminNotificationsTable(PDO $pdo): bool {
    ...
    try {
        ...
        // Upgrade an older v1/v2 table instead of relying on IF NOT EXISTS.
        ...
        $required = [
            ...
            'created_at' => "ALTER TABLE admin_notifications ADD COLUMN created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP AFTER message"
        ];
        foreach ($required as $column => $alterSql) {
            if (!in_array($column, $columns, true)) $pdo->exec($alterSql);
        }
